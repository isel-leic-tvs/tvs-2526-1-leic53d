...
int main() {
	...
	if (pid == 0) {
		...
		// The parent process will wait for the child to end.
		// If you remove this wait and insert a delay right at
		// the beginning of the child's code, you will most 
		// likely see a ppid for the child that is not its
		// original parent. Why?
		wait(NULL);
	}
